function sixthOctDecons = sixthOctDecon(convSig)
...
elseif strcmp(convSig,'vox') == 1
    ...
    centerFreqs = zeros(6,6);
    
    % create vector of base frequency for each pitch
    baseFreqs = [110 123.47 138.59 155.56 174.61 196];
    i = 1;
...
centerFreqs = reshape(centerFreqs',1,36); % this method does each note chromatically
    ...
    sixthOctDecons = cell(3,50);
       ...
       for k = 1:50 
           ...
           % decon = deconvolvedRazer{j,k};
            ...
            sixthOctDecons{j,k} = sixthFilterDecons(decon,centerFreqs);
           end
       end
end 
...
if strcmp(convSig,'test') == 1
    ...
    sixthOctDecons = cell(3,1);
    
    % pass each deconvolution of the test result to the 1/12 octave band
    % filters
    sixthOctDecons{1,1} = sixthFilterDecons(razer,centerFreqs);
    sixthOctDecons{2,1} = sixthFilterDecons(delux,centerFreqs);
    sixthOctDecons{3,1} = sixthFilterDecons(vox,centerFreqs);
    
end

end

function filterResult = sixthFilterDecons(decon,centFreqs)
    ...
    for i = 1:columns
        for k = 1:rows
         ...
         %   voxDecon = vox{k,i};
            ...
            outMatrixR(k,i) = nOctaveBandResponse(temp,fs,centFreqs(1,j),1/6);
