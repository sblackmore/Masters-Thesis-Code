...
% this is to performt he filtering of the deconvolutions on the test signal
if strcmp(convSig,'test') == 1

    filteredDecons = cell(3,1);
    
    % pass each deconvolution of the test result to the 1/12 octave band
    % filters
    filteredDecons{1,1} = filterDecons(razer,centerFreqs);
    filteredDecons{2,1} = filterDecons(delux,centerFreqs);
    filteredDecons{3,1} = filterDecons(vox,centerFreqs);
